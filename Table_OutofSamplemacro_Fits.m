...
dirname = 'Empirical_Result/macrofactor/';

%Results_GB_outofsample_IPCADATA_FNW36_RNKDMN_CON_K
name1 = 'IPCADATA_FNW36_RNKDMN_CON_KEEPFIRSTHALF_K'

%%
dirname = 'Empirical_Result/OOS/';

name1 = 'Results_GBGDmacro_outofsample_IPCADATA_FNW36_RNKDMN_CON_macro_sig_K'
...
for K=Krange
    ...
    try
    report(1,K) = ...
        1 - mySOS(xret(BIGLOC(:))-OOSRFITS_GBGD(BIGLOC(:)))/mySOS(xret(BIGLOC(:)));
    report(2,K) = ...
        1 - mySOS(xret(BIGLOC(:))-OOSRFITS_pred_GBGD(BIGLOC(:)))/mySOS(xret(BIGLOC(:)));
    report(3,K) = ...
        1 - mySOS(X(:,si:end)-OOSXFITS_GBGD(:,si:end))/mySOS(X(:,si:end));
    report(4,K) = ...
        1 - mySOS(X(:,si:end)-OOSXFITS_pred_GBGD(:,si:end))/mySOS(X(:,si:end));

...
dataname = 'IPCADATA_FNW36_RNKDMN_CON_K6'; 
oosflag = false;
QXorR = 'X';% 'Q', 'R', 'X'
annualize = false; % annualize FF -- assumes "date" vec specifies first of the twelve months over which return calc'd
ObsChoice = 'BS';% 'FF', 'SY', 'HXZ', 'BS'

% SY M4 Model?

% HXZ q factor

% BS 6factor model (SMB) (UND) (HML) (Mkt _RF) {profitability (ROE)
% investing(I/A):change in total asset / one year lagged total asset} Hou (2015)

% FF 6 factor (Mkt_RF) (SMB) (HML) (MOM)  {profitability (RMW):robust minus
% weak investing(CMA):
...
switch ObsChoice
    ...
    if ~Rlogic
    save(['Empirical_Result/OOS/' 'Results_ObsFactReg' QXorR oosstring '_' dataname oossuffix] ...
        , 'FITS*' , 'date' , 'RET')
    else
        save(['Empirical_Result/OOS/' 'Results_ObsFactReg' QXorR oosstring '_' dataname oossuffix] ...
        , 'FITS*' , 'date' , 'RET')
...
%% SY
    case 'SY'
    %% Load data
    load(['Empirical_Result/GB_result/' dataname]);
    ...
    if ~Rlogic
    save(['Empirical_Result/OOS/' 'Results_ObsFactReg_SY_' QXorR oosstring '_' dataname oossuffix] ...
        , 'FITS*' , 'date' , 'RET')
    else
        save(['Empirical_Result/OOS/' 'Results_ObsFactReg_SY_' QXorR oosstring '_' dataname oossuffix] ...
        , 'FITS*' , 'date' , 'RET')
...
%% HXZ
    ...
    hxzdata = load('HXZ_q-Factors_monthly.mat');
    ...
    if ~Rlogic
    save(['Empirical_Result/OOS/' 'Results_ObsFactReg_HXZ_' QXorR oosstring '_' dataname oossuffix] ...
        , 'FITS*' , 'date' , 'RET')
    else
        save(['Empirical_Result/OOS/' 'Results_ObsFactReg_HXZ_' QXorR oosstring '_' dataname oossuffix] ...
        , 'FITS*' , 'date' , 'RET')
...
%% BS
    ...
    bsdata = load('BarillasShanken.mat');
    ...
    if ~Rlogic
    save(['Empirical_Result/OOS/' 'Results_ObsFactReg_BS_' QXorR oosstring '_' dataname oossuffix] ...
        , 'FITS*' , 'date' , 'RET')
    else
        save(['Empirical_Result/OOS/' 'Results_ObsFactReg_BS_' QXorR oosstring '_' dataname oossuffix] ...
        , 'FITS*' , 'date' , 'RET')

mkdir Empirical_result/results_GBGDFF
%% Set estimation parameters and data choice
clear 

for K=6:6 %change here
    clearvars -except K
OFchoices = {'SY'};
for j=1:length(OFchoices)
    OFchoice = OFchoices{j}
...
%% FF models
        %ffdata = load('BarillasShanken');
        ffdata = load('M4');
        %ffdata = load('HXZ_q-Factors_monthly');

%yrmo
[date,loc1,loc2] = intersect(ffdata.dates,date);
switch OFchoice
    case 'BS'
        FF = [ffdata.Mkt_RF(loc1) ffdata.IA(loc1) ffdata.HMLm(loc1)  ffdata.ROE(loc1) ffdata.SMB(loc1) ffdata.UMD(loc1)]';
    case 'SY'
        FF = [ffdata.MKTRF(loc1) ffdata.MGMT(loc1) ffdata.PERF(loc1) ffdata.SMB(loc1)]';
    case 'HXZ'
        FF = [ffdata.Mkt_RF(loc1) ffdata.ME(loc1) ffdata.IA(loc1) ffdata.ROE(loc1)]';
end
...
%% Save results
save(['Empirical_result/results_GBGDFF/Results_GBGDFF_' dataname '_K' num2str(K) '_' OFchoice] ...
    , 'xret' , 'W' , 'date' , 'LOC' , 'Nts' ...
    , 'Gamma*' , 'Factor*' , 'Lambda*' ...
    , 'R*' ...
    , 'Q*' ...

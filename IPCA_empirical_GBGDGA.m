...
if j==1
dataname = 'IPCADATA_FNW36_keepsig_RNKDMN_CON'
% Load data
load([dataname]);
Nts = sum(LOC);
% als_opt
...
%% ALS for GBGDGA-model
% Numerical ALS procedure: starting from GB model GammaBeta guess
GB_Old      = [GammaBeta zeros(L,size(mf,1)) zeros(L,1)];
F_Old       = Factor;
tol         = 1;
iter        = 0;
tols        = nan(500,1);
...
%%
while iter<=als_opt.MaxIterations && tol>als_opt.Tolerance
    [GB_New,F_New] = num_IPCA_estimate_ALS(GB_Old,W,X,Nts,mf_1);
    tol     = max([ abs(GB_New(:)-GB_Old(:)) ; abs(F_New(:)-F_Old(:)) ]);
    F_Old   = F_New;
    GB_Old  = GB_New;
    iter    = iter+1;
...
for t=1:T
    ...
    RFITS_GBGD(:,t)      = Z(:,:,t)*(GBGA_GA + GBGD_GB*GBGD_F(:,t) + GBGD_GD*mf(:,t)) ;
    RFITS_pred_GBGD(:,t) = Z(:,:,t)*(GBGA_GA + GBGD_GB*GBGD_L + GBGD_GD*mf_L) ;
    ...
    XFITS_GBGD(:,t)      = W(:,:,t)*(GBGA_GA + GBGD_GD*mf(:,t) + GBGD_GB*GBGD_F(:,t));
    XFITS_pred_GBGD(:,t) = W(:,:,t)*(GBGA_GA + GBGD_GD*mf_L + GBGD_GB*GBGD_L);
end
...
save(['Empirical_Result/macrofactor/' dataname '_GA_K' num2str(K) '_' macrochoice] ...
    , 'xret' , 'W' , 'date' , 'LOC' , 'Nts' ...
    , 'Gamma*' , 'Factor*' , 'Lambda*' ...
    , 'R*' ...
    , 'Q*' ...

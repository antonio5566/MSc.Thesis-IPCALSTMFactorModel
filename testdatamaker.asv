...
option_keepsmall                    = 0;
option_keepbig30                    = 0;
option_keepsmall30                  = 0;
option_keepmid40                    = 0;
option_random                       = 0;
option_subsample                    = [197001 200512];% either empty or 2-vector with yyyymm specified range (eg [198001 199912])
...
max(100,(option_meandev_dataset+1)*L+option_constant+1);
sum(LOC)
%%
LOC     = LOC(:,keepthese);
...
if ~isempty(option_keepthese)
    ...
    %'IPCADATA_FNW36_keepsig_GB'
    %'IPCADATA_FNW36_keepsigmacro'
    mat_file_name='IPCADATA_FNW36_keepsig'
end
...
%% KEEPSMALL30
if option_keepsmall30
    chars = permute(chars,[1 3 2]);% make chars NxLxT
    small30 = LOC;
    for t=1:T
        tmp             = sort(chars(LOC(:,t),8,t)); %% REQUIRES SIZE TO BE THE 8th char!!!
        tmp2            = tmp(max(1,length(tmp)-999));
        small30(:,t)  = small30(:,t) & chars(:,8,t) < quantile(chars(:,8,t),0.3);
    end
    chars = permute(chars,[1 3 2]);% make chars NxTxL
        LOC = small30;
        % Check for a big enough cross section
        ...
        [N,T,L] = size(chars);
        disp(['  You keep the small stocks at ' datestr(clock)])
        mat_file_name = [mat_file_name '_KEEPSMALL30'];
end
...
if option_keepbig30
    ...
    for t=1:T
        tmp             = sort(chars(LOC(:,t),8,t)); %% REQUIRES SIZE TO BE THE 8th char!!!
        tmp2            = tmp(max(1,length(tmp)-999));
        big30(:,t)  =      big30(:,t) & chars(:,8,t) > quantile(chars(:,8,t),0.7);
    end
    chars = permute(chars,[1 3 2]);% make chars NxTxL
        LOC = big30;
        % Check for a big enough cross section
        ...
        [N,T,L] = size(chars);
        disp(['  You keep the big stocks at ' datestr(clock)])
        mat_file_name = [mat_file_name '_KEEPBIG30'];
end
...
if option_keepmid40
    ...
    for t=1:T
        tmp             = sort(chars(LOC(:,t),8,t)); %% REQUIRES SIZE TO BE THE 8th char!!!
        tmp2            = tmp(max(1,length(tmp)-999));
        big30(:,t)  =     quantile(chars(:,8,t),0.3)<= mid40(:,t) & chars(:,8,t) <= quantile(chars(:,8,t),0.7);
    end
    chars = permute(chars,[1 3 2]);% make chars NxTxL
        LOC = mid40;
        % Check for a big enough cross section
        ...
        date    = date(keepthese);
        [N,T,L] = size(chars);
        disp(['  You keep the mid stocks at ' datestr(clock)])
        mat_file_name = [mat_file_name '_KEEPMID40'];
end

%% KEEP RANDOM SPLIT
if option_random
% Randomly split the stocks into two groups
        rng('shuffle'); % Ensure random seed is based on current time
        randomSplit= LOC;
       for t=1:T
        tmp = rand(size(chars, 1), 1) < 0.5; % Adjust the probability threshold as needed
        randomSplit(:,t)= tmp
       end 
    for t=1:T
        tmp             = sort(chars(LOC(:,t),8,t)); %% REQUIRES SIZE TO BE THE 8th char!!!
        ...
        % Apply the random split to all time points
        LOC1 = randomSplit ;
        chars = permute(chars, [1 3 2]); % make chars NxTxL
        chars = chars(LOC1, :, :);
        xret = xret(LOC1, :);
        [N, T, L] = size(chars);
     mat_file_name = [mat_file_name '_KEEPRANDOM1'];
end

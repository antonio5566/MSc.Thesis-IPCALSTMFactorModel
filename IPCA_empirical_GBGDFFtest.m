...
%%
load(['Empirical_Result/results_GBGDFF/Results_GBGDFF_' dataname '_K' num2str(K) '_FF_5'])
%%
...
%%
clearvars -except R K Z X XFITS_GBGD GammaBeta Factor W FF Nts T GBGD_F GBGD_GB GBGD_GD collect Z XFITS_GB
...
%nuGBGD_GD              = zeros(L,size(mf,1));

%%

%%
boot_GB_1              = nan(bootsims,  L);
boot_GB_2              = nan(bootsims , L);
%boot_GB_3              = nan(bootsims , L);
%boot_GB_4              = nan(bootsims , L);
%boot_GB_5              = nan(bootsims , L);
%boot_GB_6              = nan(bootsims , L);
%boot_GB_7              = nan(bootsims , L);
%boot_GB_8              = nan(bootsims , L);
%boot_GB_9              = nan(bootsims , L);
%boot_GB_10              = nan(bootsims , L);
%boot_GB_11              = nan(bootsims , L);
%boot_GB_12             = nan(bootsims , L);
%boot_GB_13             = nan(bootsims , L);
%boot_GB_14              = nan(bootsims , L);
%boot_GB_15              = nan(bootsims , L);
%boot_GB_16             = nan(bootsims , L);
%boot_GB_17             = nan(bootsims , L);
%boot_GB_18              = nan(bootsims , L);
%boot_GB_19              = nan(bootsims , L);
...
if bootsims>0
    ...
    for t=1:T
    XFITS_GBGD_null(:,t)        =  W(:,:,t)*(nuGBGD_GD*FF(:,t) + GBGD_GB*GBGD_F(:,t));
    ...
    for boot=1:bootsims
        ...
        while iter<=als_opt.MaxIterations && tol>als_opt.Tolerance
            [GB_New,F_New] = num_IPCA_estimate_ALS(GB_Old,W,X_b,Nts,FF);
...
%%
result_1=mean(sum(boot_GB_1.^2,2)>mySOS(GBGD_GD(:,1:1)))
